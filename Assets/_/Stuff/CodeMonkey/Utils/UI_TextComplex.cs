    ...
    public class UI_TextComplex {
        ...
        public void AddTextOutline(Color color, float size)
        {
            foreach (Transform textComplexTrans in transform)
            {
                ...
                {
                    Shadow shadow = textComplexTrans.gameObject.AddComponent<Shadow>();
                    shadow.effectColor = color; // Correct property for Shadow color
                    shadow.effectDistance = new Vector2(size, size); // Correct property for Shadow distance
                }
            }
        }
